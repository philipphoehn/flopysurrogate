...
class FloPyEnvSurrogate():
    ...
    def compileDataset(self):
        ...
        np.save(join(wrkspc, 'dev', 'surrogateInitialXtrain_'), X)
        np.save(join(wrkspc, 'dev', 'surrogateInitialYtrain_'), Y)


        pass

        # find cells of well and neighbours
        # X unnormalize to have coords
        # env.findCells
...
if reTrain:
    ...
    for i in range(nRandomModels):
        ...
        if not recompileDataset:
            ...
            # Y = load(filehandler)
            # filehandler.close()
            X = np.load(join(wrkspc, 'dev', 'surrogateInitialXtrain_.npy'))
            Y = np.load(join(wrkspc, 'dev', 'surrogateInitialYtrain_.npy'))
        ...
        if continueModelTrain == True:
            ...
            model.add(Dense(input_dim=100, units=250))
            model.add(Activation('relu'))
            model.add(Dense(input_dim=250, units=1000))
            model.add(Activation('relu'))
            model.add(Dense(input_dim=1000, units=2500))
            model.add(Activation('relu'))
            model.add(Dense(input_dim=2500, units=lenOutput))
    ...
    if recompileDataset:
        ...
        # filehandler.close()
        np.save(join(wrkspc, 'dev', 'surrogateInitialXtest_'), Xtest)
        np.save(join(wrkspc, 'dev', 'surrogateInitialYtest_'), Ytest)
    ...
    if not recompileDataset:
        ...
        # filehandler.close()
        Xtest = np.load(join(wrkspc, 'dev', 'surrogateInitialXtest_.npy'))
        Ytest = np.load(join(wrkspc, 'dev', 'surrogateInitialYtest_.npy'))

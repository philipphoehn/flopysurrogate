...
learningRateInterval = [-3.8, -4.2]       # log scale
epsilonsInterval = [-10, -11]             # log scale
# batchSizes = [512]
batchSizes = [1024, 512, 256, 128, 64]
# batchSizes = [65536, 32768, 16384, 8192, 4096]
# activations = ['relu']
# activations = ['tanh']
# activations = ['sigmoid']
# activations = ['sigmoid', 'tanh', 'relu']
activations = ['relu', 'sigmoid', 'hard_sigmoid', 'softsign']
# activations = ['relu']
...
def collectInputAndStateFromFile(data, flagOnInitialState):
    ...
    if flagOnInitialState:
        ...
        # particle locations could be dropped but should not hurt
    ...
    if not flagOnInitialState:
        for i in range(1, len(states)-1):
            ...
            # CORRECTED THIS AS WRONG STATE WAS PREDICTED
            # particle locations could be dropped but should not hurt

            inputDataTemp = states[i-1][:-4] + stresses[i-1] + stresses[i]
            trueDataTemp = states[i][3:-4]
...
def observationsVectorToDict(observationsVector):
    ...
    observationsDict['heads'] = observationsVector[:]

...
def runGame(args):
    ...
    if not exists(join(wrkspc, 'dev')):
        mkdir(join(wrkspc, 'dev'))
    if not exists(join(wrkspc, 'dev', 'gameDataNewInitialsOnly')):
        mkdir(join(wrkspc, 'dev', 'gameDataNewInitialsOnly'))

    fileResults = join(wrkspc, 'dev', 'gameDataNewInitialsOnly', 'env' + envSettings['ENVTYPE'] + 's' + str(envSettings['SEEDENV']).zfill(9) + '.p')

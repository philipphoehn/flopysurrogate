...
modelName = 'surrogateModelStatesRewardsTest'
trainSamplesLimit = None
testSamplesLimit = 50000
learningRate = 0.01
batch_size = 32784
epochs = 10000
patience = 10
...
if recompileDataset:
    ...
    for data in filePerGame:
        ...
        for i in range(len(states)-1):
            ...
            Y.append(stateNext)
            Yrewards.append(rewardNext)

    X = np.asarray(X)
    Y = np.asarray(Y)
    Yrewards = np.asarray(Yrewards)

    # naive approach to normalize using max
    Yrewards = divide(Yrewards, env.rewardMax)
    ...
    dump(Y, filehandler)
    filehandler.close()
    filehandler = open(join(wrkspc, 'dev', 'surrogateYrewardstrain.p'), 'wb')
    dump(Y, filehandler)
    filehandler.close()
...
if reTrain:
    if not recompileDataset:
        ...
        Y = load(filehandler)
        filehandler.close()
        filehandler = open(join(wrkspc, 'dev', 'surrogateYrewardstrain.p'), 'rb')
        Y = load(filehandler)
        filehandler.close()
    ...
    if trainSamplesLimit != None:
        ...
        Yrewards = Yrewards[0:trainSamplesLimit]
    ...
    reward = Dense(1, name='reward')(hl3)
    ...
    sampleweightsRewards = calculate_weightsforregression(Yrewards, nbins=30)
    ...
    results = model.fit(X, [Y[:, i] for i in range(lenInput)] + Yrewards, sample_weight=[sampleweightsAll] + sampleweightsRewards,
...
if recompileDataset:
    ...
    Xtest = np.asarray(X)
    Ytest = np.asarray(Y)
    Yrewardstest = np.asarray(Yrewards)

    # naive approach to normalize using max
    Yrewardstest = divide(Yrewardstest, env.rewardMax)
    ...
    filehandler.close()
    filehandler = open(join(wrkspc, 'dev', 'surrogateYrewardstest.p'), 'wb')
    dump(Yrewardstest, filehandler)
    filehandler.close()
...
if not recompileDataset:
    ...
    filehandler.close()
    filehandler = open(join(wrkspc, 'dev', 'surrogateYrewardstest.p'), 'rb')
    Yrewardstest = load(filehandler)
    filehandler.close()
...
if reTest:
    ...
    for i in range(len(Xtest)):
        ...
        prediction = env.observationsVectorToDict(prediction)

...
learningRateInterval = [-3.0, -5.0]         # log scale
epsilonsInterval = [-10, -11]            # log scale
# batchSizes = [512]
batchSizes = [16384, 4096, 2056, 1024, 512, 256]
# batchSizes = [65536, 32768, 16384, 8192, 4096]
# activations = ['relu']
# activations = ['tanh']
# activations = ['sigmoid']
# activations = ['sigmoid', 'tanh', 'relu']
activations = ['relu', 'selu', 'elu', 'sigmoid']
# activations = ['relu']
...
kernelInitializers = ['glorot_uniform', 'he_uniform']
...
def collectInputAndStateFromFile(data, flagOnInitialState):
    ...
    if not flagOnInitialState:
        for i in range(1, len(states)-1):
            ...
            inputDataTemp = states[i-1][:-4] + stresses[i-1] + states[i][:-4] + stresses[i]

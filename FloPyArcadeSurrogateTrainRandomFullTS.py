...
# nLayers_ = [6, 5, 4, 3, 2]
nLayers_ = [2]
nNeuronsInterval = [3, 3.2]               # log scale
learningRateInterval = [-3.5, -3.5]         # log scale
epsilonsInterval = [-10, -11]            # log scale
# batchSizes = [512]
batchSizes = [16384, 4096, 512]
...
def collectInputAndStateFromFile(data, NAGENSTEPS):
    ...
    # initial timesteps
    inputData_nValuesInitialTimeStep = len(states[0][0:3]) + len(stresses[0])
    # trueData_nValuesInitialTimeStep = len(states[0][3:-4])
    # following timesteps
    inputData_nValuesPerTimeSteps = 2*len(states[0][:-4]) + 2*len(stresses[0])
    trueData_nValuesPerTimeSteps = len(states[0][:-4])
    # generating list of zeros
    # print(inputData_nValuesInitialTimeStep, trueData_nValuesInitialTimeStep, inputData_nValuesPerTimeSteps, trueData_nValuesPerTimeSteps)
    inputData_nTotalValues = inputData_nValuesInitialTimeStep + (NAGENSTEPS-1)*inputData_nValuesPerTimeSteps
    # trueData_nTotalValues = trueData_nValuesInitialTimeStep + (NAGENSTEPS-1)*trueData_nValuesPerTimeSteps
    # trueData_nTotalValues = trueData_nValuesInitialTimeStep + trueData_nValuesPerTimeSteps
    trueData_nTotalValues = trueData_nValuesPerTimeSteps


    nTimeSteps = len(states)
    # browsing through timesteps
    for length in range(nTimeSteps-1):
        inputDataTemp = [0. for i in range(inputData_nTotalValues)]
        trueDataTemp = [0. for i in range(trueData_nTotalValues)]
        inputData_count, trueData_count = 0, 0
        for i in range(len(states[0][0:3])):
            # print(inputData_nTotalValues, trueData_nTotalValues, inputData_count, i)
            inputDataTemp[inputData_count] = states[0][i]
            inputData_count += 1
        for i in range(len(stresses[0])):
            inputDataTemp[inputData_count] = stresses[0][i]
            inputData_count += 1

        if length == 0:
            trueDataTemp[trueData_count] = states[0][0]
            trueData_count += 1
            trueDataTemp[trueData_count] = states[0][1]
            trueData_count += 1
            trueDataTemp[trueData_count] = states[0][2]
            trueData_count += 1
            for i in range(len(states[0][3:-4])):
                trueDataTemp[trueData_count] = states[0][3+i]
                trueData_count += 1

        if length > 0:
            for step in range(1, length+1):
                # print('step', step, 'length', length)
                for i in range(len(states[step-1][:-4])):
                    inputDataTemp[inputData_count] = states[step-1][i]
                    inputData_count += 1
                for i in range(len(stresses[step-1])):
                    inputDataTemp[inputData_count] = stresses[step-1][i]
                    inputData_count += 1
                for i in range(len(states[step][:-4])):
                    inputDataTemp[inputData_count] = states[step][i]
                    inputData_count += 1
                for i in range(len(stresses[step])):
                    inputDataTemp[inputData_count] = stresses[step][i]
                    inputData_count += 1

            for i in range(len(states[step+1][:-4])):
                trueDataTemp[trueData_count] = states[step+1][i]
                trueData_count += 1

        # trueDataTemp too long?

        # print('len(inputDataTemp)', len(inputDataTemp))
        # print('len(trueDataTemp)', len(trueDataTemp))
        # print('####')
